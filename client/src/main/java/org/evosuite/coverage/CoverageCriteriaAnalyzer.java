...
public class CoverageCriteriaAnalyzer {
    ...
    public static Map<Integer, Integer> validateBranchCoveredCount(Map<Integer, Integer> map, List<TestFitnessFunction> ff) {
    	Map<Integer, Integer> finalExecCount = new TreeMap<Integer, Integer>();
		for(int i=0; i<ff.size(); i++) {
			if(ff.get(i).toString().contains("root-Branch")) {
				finalExecCount.put((i+1), 0);
			}else {
				String[] line = ff.get(i).toString().split(" ");
				int branch = 0;
				for(int j=0; j<line.length; j++) {
					if(line[j].equals("Branch")) {
						branch = Integer.valueOf(line[j+1]);
					}
				}
				String bool = line[line.length-1];
				
				int predBranch = Integer.valueOf(String.valueOf(branch) + (bool.equals("true")? 1:2));
				finalExecCount.put((i+1), map.get(predBranch));
			}
		}
		return finalExecCount;
    }
    
    public static void writeToCSV(String valType , Map<Integer, Integer> result) {
		File outputDir = getReportDir();
		
		String class_name = Properties.TARGET_CLASS;
		
		String file_name = class_name + "_" + valType + "_coverage.csv";
		...
		try {
			...
			for(int i=0; i<result.size(); i++) {
				out.write(String.valueOf(result.get(keys[i])));
				out.write(",");
			}
