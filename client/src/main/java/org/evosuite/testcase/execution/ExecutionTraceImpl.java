...
public class ExecutionTraceImpl implements ExecutionTrace, Cloneable {
	...
	public Map<Integer, Integer> mergedBranches = new HashMap<Integer, Integer>();
	...
	private String methodThrowException = null;
	...
	/**
	 * True and false branches that are stored in coveredTrue and coveredFalse will be merged in one map
	 * For example, the true and false branches for predicate #5 will be stored like (51,3) and (52,0)
	 * where 51 is the true branch and 3 is the number of executions for this branch and 52 is the false branch
	 * that has not been executed
	 */
	private void mergeTrueandFalseBranches(){
		for(int predicateID=1; predicateID<=DependencyAnalysis.getPredicates(); predicateID++){
			for(int flowID=1; flowID<=2; flowID++){ // 1 is true branch and 2 is false branch 
				int branchID = Integer.valueOf(String.valueOf(predicateID)+flowID);
				if(flowID==1)
					mergedBranches.put(branchID, coveredFalse.containsKey(predicateID) ? coveredFalse.get(predicateID) : 0);
				else
					mergedBranches.put(branchID, coveredTrue.containsKey(predicateID) ? coveredTrue.get(predicateID) : 0);
			}
		}
	}
	
	@Override
	public Map<Integer, Integer> getBranchExecutionCount() {
		this.mergeTrueandFalseBranches();
		return mergedBranches;
	}

	@Override
	public void setMethodThrowException(String methodName) {
		methodThrowException = methodName;
	}
